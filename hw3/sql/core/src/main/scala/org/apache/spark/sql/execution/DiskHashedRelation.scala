...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    ...
    if (inputClosed){
      ...
      throw new SparkException("Should not get data if input is closed. Bad things will happen!")
    }
    data.add(row)
    Int size_partition = measurePartitionSize()
    if (size_partition > blockSize){
      this.spillPartitionToDisk()
      data = new JavaArrayList[Row]
    }
    // else{
    // data.add(row)
    // }
  ...
  private[this] def spillPartitionToDisk() = {
    ...
    Files.write(path, bytes, StandameasurePartitionSizerdOpenOption.APPEND)
    writtenToDisk = true
  }
  ...
  def closeInput() = {
    ...
    //if data not written to disk yet
    if (data.length() == 0){
      this.spillPartitionToDisk() //write to the disk
      outStream.close()
    }
    inputClosed = true
  }
